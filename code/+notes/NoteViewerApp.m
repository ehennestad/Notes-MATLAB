...
%   
%   See also notes.Note notes.NoteBook
...
%   [ ] Methods for sorting, filtering and reseting list & tree
    ...
    methods (Access = public)
        ...
        function app = NoteViewerApp(notes, varargin)
            ...
            drawnow
            utility.layout.setUiFigureMinSize(app.UIFigure, [700,400])
    ...
    methods (Access = protected)
        ...
        %updateTypeSelectionList Update items in type selection dropdown    
            items = ['Show All', notes.Note.VALID_NOTE_TYPES];
    ...
    methods % Public methods
        ...
        function assignNotebook(app, noteArray)
        %assignNotebook Assign notebook in various forms.    
            
            if isa(noteArray, 'notes.Note')
                app.Notebook = notes.NoteBook( noteArray );
            elseif isa(noteArray, 'notes.NoteBook')
                app.Notebook = noteArray;
            elseif isa(noteArray, 'struct')
                app.Notebook = notes.NoteBook( noteArray );
            else
                errorId = 'Notes:NoteViewerApp:InvalidInput';
